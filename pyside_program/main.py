...
def bp_rasshet():
    ...
    if value_vattmeter.isdigit() or (value_vattmeter.count('.') == 1 and value_vattmeter.replace('.', '').isdigit()):
        ...
        if input_atten_power:
            if input_atten_power.isdigit() or (input_atten_power.count('.') == 1 and input_atten_power.replace('.', '').isdigit()):
        ...
        if input_atten_ampl:
            if input_atten_ampl.isdigit() or (input_atten_ampl.count('.') == 1 and input_atten_ampl.replace('.', '').isdigit()):
        ...
        pulse_power_ceil = (value_vattmeter * attenuation_tract) / (signal_duration_floor * 10**(-6) * pulse_repetition_rate * (performance_ratio_Ke_floor - 4*coaxial_transition_utilization_rate_a**2) * (1 - converter_reflectance_module**2))
        pulse_power_floor = (value_vattmeter * attenuation_tract) / (signal_duration_ceil * 10**(-6) * pulse_repetition_rate * (performance_ratio_Ke_ceil - 4*coaxial_transition_utilization_rate_a**2) * (1 - converter_reflectance_module**2))
